...
class InterfaceGenerator {
    ...
    private fun generateStatementFunctions() {
        val randomStatementFunction = FunSpec.builder("selectRandomStatement")
            ...
            .build()
        val codeBlock = CodeBlock.builder().beginControlFlow("return when(%N(%N))", randomStatementFunction, "selectionManager")
        Statement::class.subclasses().filter { it.hasAnnotation<GenNode>() || it.hasAnnotation<ExpressionGenNode>() }.forEach {
    ...
    private fun generateExpressionFunctions() {
        val randomExpressionFunction = FunSpec.builder("selectRandomExpression")
            ...
            .addModifiers(KModifier.ABSTRACT)
            .build()
        val codeBlock = CodeBlock.builder().beginControlFlow("return when(%N(%N, %N))", randomExpressionFunction, "type", "selectionManager")
        Expression::class.subclasses().filter { it.hasAnnotation<ExpressionGenNode>() }.toSet().forEach {
            val funSpec = FunSpec.builder("generate${it.simpleName}")
                .addParameter(ParameterSpec.builder("type", Type::class.asClassName()).build())
                .addParameter(ParameterSpec.builder("selectionManager", SelectionManager::class).build())
                .returns(it)
